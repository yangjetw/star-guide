...
test('defines the Traditional Chinese HTML page contract', async () => {
  ...
  assert.match(html, /<link\s+rel=["']stylesheet["']\s+href=["']styles\.css\?v=20260725-8["']\s*\/?>/i);
...
test('loads the warm stylesheet cache key on every public page', async () => {
  for (const page of ['index.html', 'gift.html', 'navigator.html', 'refund.html']) {
    assert.match(await read(page), /<link\s+rel=["']stylesheet["']\s+href=["']styles\.css\?v=20260725-8["']\s*\/?>/i, `${page} should load the warm stylesheet cache key`);
  }
});
